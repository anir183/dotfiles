...
function setup_arch_packages() {
	if eval $check_arch; then
		...
		local treesitter_cmd="distrobox enter $container_name -- npm install tree-sitter-cli --global"
		cmd_run $pkg_install_cmd 2
		cmd_run $treesitter_cmd 2
	else
		echo "created container is not an arch image"
...
function record_setup_container() {
	...
	distrobox list | grep "| $container_name " | cut -d ' ' -f1|tr ' ' ' ' >> $XDG_STATE_HOME/devbox/setup_containers
}
...
function run_container() {
	...
	vared -p "<enter> or <y> to run container: " -c choice
	case $(rmv_all_spaces $choice) in
		"" )
			;&
		[yY]* )
			. ./run.zsh setup $container_name
			;;
		* )
			echo "exiting without running"
			echo "can be manually run with: devbox run"
			;;
	esac
}
