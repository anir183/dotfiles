...
function Get-Theme_Override {
    ...
    $localThemePath = Join-Path (Get-ProfileDir) "catppuccin_frappe.omp.json"
    if (-not (Test-Path $localThemePath)) {
        # Try to download the theme file to the detected local path
        $themeUrl = "https://raw.githubusercontent.com/JanDeDobbeleer/oh-my-posh/refs/heads/main/themes/catppuccin_frappe.omp.json"
        try {
